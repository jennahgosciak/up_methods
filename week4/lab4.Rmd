...
create_table <- function(df, v = 1) {
  ...
  tbl_exp <- df1 %>% 
    ...
    kbl(align=c(rep('l',times=(ncol(df1)-1))), format = "latex",
        col.names = c(" ", "Category",
                      rep(c("Count of Lots", "Share of Lots"), ((ncol(df1) - 2)/2)))) %>% 
    kable_styling(position = "left",
                  latex_options = c("hold_position")) %>%
    column_spec(1, width = "7em", bold = TRUE, border_left = TRUE) %>% 
    column_spec(2, width = "10em") %>% 
    column_spec(3:(ncol(df1) - 1), width = "5em") %>% 
    column_spec(ncol(df1), width = "4.5em", border_right = TRUE) %>% 
